  ...
  useEffect(() => {
    ...
    class AutoDriver {
      ...
      update() {
        ...
        const dir = this._tmpDir.subVectors(this.target, this.coords);
        ...
        this.coords_old.copy(this.coords);
        this.coords.add(dir.multiplyScalar(move));
        this.diff.subVectors(this.coords, this.coords_old);
      }
    }
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps: any) {
        ...
        const createMat = () => new THREE.RawShaderMaterial({
        ...
        this.mouse = new THREE.Mesh(mouseG, createMat());
        this.scene!.add(this.mouse);

        this.autoMouse = new THREE.Mesh(mouseG, createMat());
        this.scene!.add(this.autoMouse);
      }
      update(...args: any[]) {
        const props = args[0] || {};
        ...
        // Auto Mouse
        const autoDriver = props.autoDriver;
        {
          const uniforms = (this.autoMouse.material as THREE.RawShaderMaterial).uniforms;
          if (autoDriver && autoDriver.enabled) {
            const forceX = (autoDriver.diff.x / 2) * (props.mouse_force || 0);
            const forceY = (autoDriver.diff.y / 2) * (props.mouse_force || 0);
            const centerX = Math.min(Math.max(autoDriver.coords.x, -1 + cursorSizeX + cellScale.x * 2), 1 - cursorSizeX - cellScale.x * 2);
            const centerY = Math.min(Math.max(autoDriver.coords.y, -1 + cursorSizeY + cellScale.y * 2), 1 - cursorSizeY - cellScale.y * 2);
            uniforms.force.value.set(forceX, forceY);
            uniforms.center.value.set(centerX, centerY);
            uniforms.scale.value.set(cursorSize, cursorSize);
          } else {
    ...
    class Output {
      ...
      update(autoDriver?: any) {
        this.simulation.update(autoDriver);
        this.render();
      }
    }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      render() {
        ...
        Mouse.update();
        Common.update();
        this.output.update(this.autoDriver);
      }
      loop() {
